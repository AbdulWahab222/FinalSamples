https://testnet.bscscan.com/address/0x67bfbff03be4d56a90f334e68f06537f0c2da350#code
...
contract testKIN is ERC20, Ownable {
    ...
    uint256 public maxBuyTransactionAmount = 1000000000000 * (10**18);
    ...
    uint256 public devBuyFee = 3;
    uint256 public totalBuyFees = 10;

    uint256 public buyBackSellFee = 3;
    uint256 public marketingSellFee = 7;
    uint256 public charitySellFee = 3;
    uint256 public devSellFee = 7;
    uint256 public totalSellFees = 20;
 
    address payable public marketingWallet = payable(0x360972858b35F71a88d4cC4AECb365dF07A2aFeb);
    address payable public devWallet = payable(0xC22DD392cE42C2cd9421fcE51b90Eb39010a8AD7);
    address payable public charityWallet = payable(0xB0Bba77e9b546bBc17B3B2Aa5356AbC13970D062);
    ...
    function setBuyFees(uint256 _buybackFee, uint256 _marketingFee, uint256 _charityFee, uint256 _devFee) public onlyOwner {  
        buyBackBuyFee = _buybackFee;
        marketingBuyFee = _marketingFee;
        charityBuyFee = _charityFee;
        devBuyFee = _devFee;
        totalBuyFees = buyBackBuyFee.add(marketingBuyFee).add(charityBuyFee).add(devBuyFee); 
    }

    function setSellFees(uint256 _buybackFee, uint256 _marketingFee, uint256 _charityFee, uint256 _devFee) public onlyOwner {  
        buyBackSellFee = _buybackFee;
        marketingSellFee = _marketingFee;
        charitySellFee = _charityFee;
        devSellFee = _devFee;
        totalSellFees = buyBackSellFee.add(marketingSellFee).add(charitySellFee).add(devSellFee); 
    }
    ...
    constructor() ERC20("TestKingkong Inu", "TestKIN") {
    	IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3);//0xF491e7B69E4244ad4002BC14e878a34207E38c29);
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "KIN: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "KIN: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "KIN: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "KIN: Automated market maker pair is already set to that value");
    ...
    function _transfer(
        ...
        if(!inSwapAndLiquify && !automatedMarketMakerPairs[from] && swapAndLiquifyEnabled) {
        ...
        if(!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            uint256 fees = (amount*totalBuyFees)/100;
    ...
    function swapAndSendToWallets(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 marketingShare = balanceAfterSwap.mul(marketingSellFee).div(totalSellFees);
        uint256 charityShare = balanceAfterSwap.mul(charitySellFee).div(totalSellFees);
        uint256 devShare = balanceAfterSwap.mul(devSellFee).div(totalSellFees);
