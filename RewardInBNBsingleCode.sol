//https://bscscan.com/token/0xfa19e2e46285608f1a4e14308e9a1195be3feac2
...
contract DOGENFT is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public maxBuyTransactionAmount = 900000000000 * (10**18);
    uint256 public maxSellTransactionAmount = 900000000000 * (10**18);
    ...
    address payable public  marketingWallet = payable(0x4AE7551d289E97DF238478EC3107A206e04e8306);
    ...
    constructor() ERC20("DOGE NFT FUND", "DOGENFT") {
        ...
        */
        _mint(owner(), 900000000000 * (10**18));
    }
...
//this will be used to exclude from dividends the presale smart contract address
    ...
    function _transfer(
        ...
        if(!excludedAccount && (automatedMarketMakerPairs[from] || automatedMarketMakerPairs[to])) {
        	uint256 swapableFees = amount.mul(BNBRewardsFee.add(liquidityFee).add(marketingFee)).div(100);
            uint256 burnShare = amount.mul(burnFee).div(100);
        	amount = amount.sub(swapableFees.add(burnShare));

            if(swapableFees > 0) {
                super._transfer(from, address(this), swapableFees);
            }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee);
        ...
        uint256 tokensForHolders = contractTokenBalance.sub(tokensForLiquidity.add(tokensForMarketing));
    ...
    function updateFees(uint256 _bnbRewardFee, uint256 _liquidityFee, uint256 _marketingFee, uint256 _burnFee) public onlyOwner {
        require(_bnbRewardFee.add(_liquidityFee).add(_marketingFee).add(_burnFee) <= 19, "Fees too high");
    ...
    function setMaxBuyTxAmount(uint256 _maxBuyTxAmount) public onlyOwner {
        ...
        require(maxBuyTransactionAmount >= totalSupply().div(1000), "value too low");
    }

    function setMaxSellTxAmount(uint256 _maxSellTxAmount) public onlyOwner {   
        maxSellTransactionAmount = _maxSellTxAmount;
        require(maxSellTransactionAmount >= totalSupply().div(1000), "value too low");
    }
