// https://bscscan.com/token/0xd5a7f5d725467896d232f12c8dca099e633fe855
...
contract Luna is Context, IERC20, Ownable {
    ...
    string private _name = "Terra Luna 2.0";
    ...
    address payable public marketingWallet = payable(0xacb051C1Df0162B185Fd162bB7c2FD1a0aeE828c);
    
    uint256 public buybackDivisor = 50;
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _buybackFee==0 && _marketingFee==0) return;
    ...
    function _transfer(
        ...
        if (
            ...
            if(overMinTokenBalance) {
                ...
                //add liquidity and swap for buyback
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 liquidityShare = contractTokenBalance.mul(_liquidityFee).div(_liquidityFee.add(_buybackFee).add(_marketingFee));
        ...
        //swap for marketing and buyback
        swapTokensForEth(contractTokenBalance.sub(liquidityShare));

        uint256 balanceAfterSwap = address(this).balance.sub(balanceBeforeSwap);

        // transfer to marketing wallet
        marketingWallet.transfer(balanceAfterSwap.mul(_marketingFee).div(_marketingFee.add(_buybackFee)));
    ...
    function setBuyBackUpperLimit(uint256 newAmount) external onlyOwner() {
        buyBackUpperLimit = newAmount;
    }
    
    function setBuybackDivisor(uint256 newValue) external onlyOwner() {
        require(buybackDivisor > 0);
        buybackDivisor = newValue;
    }
    
     function setFeePercent(uint256 taxFee, uint256 liquidityFee, uint256 buybackFee, uint256 marketingFee) external onlyOwner() {
        require(taxFee.add(liquidityFee).add(buybackFee).add(marketingFee) <= 20, "tax too high");
        _taxFee = taxFee;
