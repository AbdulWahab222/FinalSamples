//https://bscscan.com/token/0xacdda588bf725241a4db919e7f17d0a2a9c3a16d
//https://testnet.bscscan.com/token/0xd0887016dc75d772409214ab853593d6669cf210  //testnet
...
contract Truffle is Context, IERC20, Ownable {
    ...
    string private _name = "Truffle";
    string private _symbol = "TRUFF";
    uint8 private _decimals = 18;
    
    address public deadAddress = 0x000000000000000000000000000000000000dEaD;
    address payable public charityWallet = payable(0x08A5da74322f43a3023D855F0412cA38d9aBCA26);
    ...
    //Sale Fee
    uint256 public _saleTaxFee = 5;
    uint256 public _saleLiquidityFee = 8;
    uint256 public _saleCharityFee = 6;
    uint256 public _saleBuyBackFee = 1;
    ...
    uint256 public minimumTokensBeforeSwap = 100000 * 10**18;
    uint256 public _maxBuyTxAmount = 1000000000 * 10**18;
    uint256 public _maxSellTxAmount = 1000000000 * 10**18;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 liquidityTokenShare = contractTokenBalance.mul(_saleLiquidityFee).div(
            _saleLiquidityFee.add(_saleBuyBackFee).add(_saleCharityFee));
        ...
        swapTokensForEth(contractTokenBalance.sub(liquidityTokenShare));
        // calculate recently swaped BNB value
        uint256 deltaBalance = address(this).balance.sub(balanceBeforeSwap);
        uint256 charityBNBShare = deltaBalance.mul(_saleCharityFee).div(_saleCharityFee.add(_saleBuyBackFee));
        charityWallet.transfer(charityBNBShare);
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private 
    {
        ...
        {
            setAllFees(_saleTaxFee, _saleLiquidityFee, _saleCharityFee, _saleBuyBackFee);
        }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private 
    {
        ...
        (tTransferAmount, rTransferAmount) = takeBuyBack(sender, tTransferAmount, rTransferAmount, tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);
        _takeLiquidity(tLiquidity);
        _reflectFee(rFee, tFee);
    ...
    function takeBuyBack(address sender, uint256 tTransferAmount, uint256 rTransferAmount, uint256 tAmount) private
    returns (uint256, uint256)
    {
        if(_buyBackFee==0) {  return(tTransferAmount, rTransferAmount); }
        uint256 tBuyBack = tAmount.div(100).mul(_buyBackFee);
        uint256 rBuyBack = tBuyBack.mul(_getRate());
        rTransferAmount = rTransferAmount.sub(rBuyBack);
    ...
    function setAllFees(uint256 taxFee, uint256 liquidityFee, uint256 charityFee, uint256 buyBackFee) private {
        require(taxFee.add(liquidityFee).add(charityFee).add(buyBackFee) <= 20, "tax too high");
        _taxFee = taxFee;
    ...
    function setFees(uint256 taxFee, uint256 liquidityFee, uint256 charityFee, uint256 buyBackFee) 
    external onlyOwner() {
        require(taxFee.add(liquidityFee).add(charityFee).add(buyBackFee) <= 20, "tax too high");
        setAllFees(taxFee, liquidityFee, charityFee, buyBackFee);
    ...
    function setSaleFees(uint256 taxFee, uint256 liquidityFee, uint256 charityFee, uint256 buyBackFee) 
    external onlyOwner() {
        require(taxFee.add(liquidityFee).add(charityFee).add(buyBackFee) <= 20, "tax too high");
        _saleTaxFee = taxFee;
    ...
    function setMinimumTokensBeforeSwap(uint256 newAmt) external onlyOwner() {
     ...
     function setMaxBuyTxAmount(uint256 maxBuyTxAmount) external onlyOwner() {
        _maxBuyTxAmount =  maxBuyTxAmount;
        require(_maxBuyTxAmount >= totalSupply().div(200), "value too low");
    }
    
    function setMaxSellTxAmount(uint256 maxSellTxAmount) external onlyOwner() {
        _maxSellTxAmount = maxSellTxAmount;
        require(_maxSellTxAmount >= totalSupply().div(200), "value too low");
    }
