//https://testnet.bscscan.com/address/0x809342d9389d3533356cfff3931e343236211838#code
...
contract SVDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("SV_Dividend_Tracker", "SV_Dividend_Tracker") {
        claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "SV_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "SV_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SV contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "SV_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "SV_Dividend_Tracker: Cannot update claimWait to same value");
...
contract testSV is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    SVDividendTracker public dividendTracker;
    ...
    uint256 public totalFees = BNBRewardsFee.add(liquidityFee).add(buybackFee).add(marketingFee).add(devFee); 
    
    address payable public  marketingWallet = payable(0xcC95b2B74Ae19c9Ba64F8Bb6041F73914c5646C0);
    address payable public  devWallet = payable(0x3236AA376c356D710fBA532B13A4aa762c8A5493);
    address public immutable deadAddress = 0x000000000000000000000000000000000000dEaD;
    
    uint256 private buyBackUpperLimit = 1 * 10**17;
    uint256 private _buyBackDivisor = 10;
    ...
    function setFees(uint256 _bnbRewardFee, uint256 _liquidityFee,  uint256 _buybackFee,  uint256 _marketingFee, uint256 _devFee) 
    public onlyOwner 
    {
        ...
        devFee = _devFee;
        totalFees = BNBRewardsFee.add(liquidityFee).add(buybackFee).add(marketingFee).add(devFee); // total fee
    }
    
    function setMaxTxAmount(uint256 _maxBuyTxAmount, uint256 _maxSellTxAmount) public onlyOwner {
        maxBuyTransactionAmount = _maxBuyTxAmount * (10**18);
        maxSellTransactionAmount = _maxSellTxAmount * (10**18);
    }

    constructor() ERC20("testSECURITIZED VENTURES", "test$SV") 
    {
    
    	dividendTracker = new SVDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "SV: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "SV: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "SV: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "SV: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "SV: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "SV: Cannot update gasForProcessing to same value");
...
//this will be used to exclude from dividends the presale smart contract address
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 tokensToAddLiquidityWith = contractTokenBalance.mul(liquidityFee).div(totalFees).div(2);
        ...
        uint256 dividends = deltaBalance.mul(BNBRewardsFee).div(totalFees);
    ...
    function SetBuyBackUpperLimitAmount(uint256 _newLimit) public {
    ...
    function setBuyBackDivisor(uint256 _divisor) public onlyOwner {
        _buyBackDivisor = _divisor;
    }
    ...
    function setDevWallet(address payable newWallet) public onlyOwner {
        devWallet = newWallet;
    }
